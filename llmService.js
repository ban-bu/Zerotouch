// [MODIFIED] ModelScope LLM service (legacy-compatible API)
...
// Generic function to call ModelScope API
const callModelScopeAPI = async (messages, temperature = 0.7) => {
  try {
    ...
    console.error('ModelScope API call error:', error)
    throw error
  }
}
...
const sanitizeOutput = (text) => {
  ...
  const bannedPhrases = [
    ...
    'Are you currently looking for a particular type of product',
    'Clothes, shoes, or other items'
  ]
  let sanitized = text
...
// Process problem input - enhanced version, supports chat history and deep understanding
const processProblemInput = async (content, image, scenario, chatHistory = []) => {
  try {
    // Scenario-specific prompts - enhanced version
    ...
    // Extract Information to Confirm
    ...
    // Build detailed steps for UI display
    const steps = [
      {
    ...
    if (sections.solutions.length > 0) {
      steps.push({
        ...
        content: sections.solutions.map((solution, index) => `Option ${index + 1}: ${solution}`).join('\n\n')
      })
    }
    ...
    // Construct message to solution side: only include needs translation
...
// Process solution response - enhanced version, supports chat history and optimization
...
const analyzeContext = async (content) => {
  const prompt = [
    {
      role: 'system',
      content: 'You are a context analysis expert. Analyze the business scenario and context of the user input.'
    },
    {
      role: 'user',
      content: `User input: "${content}"\n\nPlease analyze the possible business scenario, industry background, or usage context.`
    }
  ]
...
const conceptualize = async (content) => {
  const prompt = [
    {
      role: 'system',
      content: 'You are a concept design expert. Convert the user\'s need into concrete concepts and functional points.'
    },
    {
      role: 'user',
      content: `Based on the user input: "${content}"\n\nPlease conceptualize it into specific functional requirements or solution bullets.`
    }
  ]
  return await callModelScopeAPI(prompt)
}
...
const detectMissingInfo = async (content) => {
  const prompt = [
    {
      role: 'system',
      content: 'You are a requirements completeness expert. Identify key information that may be missing from the input.'
    },
    {
      role: 'user',
      content: `User input: "${content}"\n\nWhich additional information is needed to better understand and fulfill the user\'s needs?`
    }
  ]
  return await callModelScopeAPI(prompt)
}
...
const translateToSolution = async (content) => {
  const prompt = [
    {
      role: 'system',
      content: 'You are a requirement translation expert. Convert the raw input into a clear, professional requirement description.'
    },
    {
      role: 'user',
      content: `Original user input: "${content}"\n\nPlease convert it into a clear, professional requirement description with concrete functional needs and expected outcomes.`
    }
  ]
  return await callModelScopeAPI(prompt)
}
...
const optimizeForUser = async (content) => {
  const prompt = [
    {
      role: 'system',
      content: 'You are a user experience expert. Convert the technical plan into user‑friendly language and provide clear action guidance.'
    },
    {
      role: 'user',
      content: `Technical plan: "${content}"\n\nPlease convert it into user‑friendly language with clear steps and expected outcomes.`
    }
  ]
  return await callModelScopeAPI(prompt)
}
